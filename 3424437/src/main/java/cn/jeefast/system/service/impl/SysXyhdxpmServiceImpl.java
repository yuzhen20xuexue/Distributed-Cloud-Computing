...
import cn.jeefast.system.dao.SysXyhdxpmDao;
 ...
 */
@Service
public class SysXyhdxpmServiceImpl extends ServiceImpl<SysXyhdxpmDao, SysXyhdxpm> implements SysXyhdxpmService {
    @Autowired
    private SysXyhdxpmDao sysXyhdxpmDao;

    @Override
    public Page<SysXyhdxpm> queryPageList(Page<SysXyhdxpm> page, Map<String, Object> map) {
        page.setRecords(sysXyhdxpmDao.queryPageList(page, map));
        return page;
    }
    public Page<SysXyhdxpm> queryPagecolList(Page<SysXyhdxpm> page, Map<String, Object> map) {
        page.setRecords(sysXyhdxpmDao.queryPagecolList(page, map));
        return page;
    }

    @Override
    public void deleteBatch(String[] xyhdxpmIds) {
        sysXyhdxpmDao.deleteBatch(xyhdxpmIds);
    }
    ...
    public List<Map<String, Object>> getList(String groupId) {
        ...
        List<SysXyhdxpm> sysXyhdxpms = sysXyhdxpmDao.selectList(new EntityWrapper<SysXyhdxpm>().orderBy(true,"updatetime"));
        int i =0;
        for(SysXyhdxpm sysXyhdxpm : sysXyhdxpms) {
            Map<String, Object> GroupXnMap = new HashMap<String, Object>();
//            GroupXnMap.put("xu", ++i);
            GroupXnMap.put("xh", sysXyhdxpm.getXh());
            GroupXnMap.put("xxmc", sysXyhdxpm.getXxmc());
            GroupXnMap.put("xuexiao", sysXyhdxpm.getXuexiao());
            GroupXnMap.put("zf", sysXyhdxpm.getZf());
            GroupXnMap.put("xj", sysXyhdxpm.getXj());
            GroupXnMap.put("bxcc", sysXyhdxpm.getBxcc());
...
//		String[] firs = { ",0,3", "联系走访,4,10", "政策宣讲,11,16", "产业投资,17,18", "扶贫投资"};
        String[] firs = { "校友会大学排名管理表,0,6", ",7,8"};
