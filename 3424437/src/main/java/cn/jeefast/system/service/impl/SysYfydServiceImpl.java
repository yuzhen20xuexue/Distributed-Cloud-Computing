...
import cn.jeefast.system.dao.SysYfydDao;
 ...
 */
@Service
public class SysYfydServiceImpl extends ServiceImpl<SysYfydDao, SysYfyd> implements SysYfydService {
    @Autowired
    private SysYfydDao sysYfydDao;

    @Override
    public Page<SysYfyd> queryPageList(Page<SysYfyd> page, Map<String, Object> map) {
        page.setRecords(sysYfydDao.queryPageList(page, map));
        return page;
    }

    @Override
    public Page<SysYfyd> queryPagecolList(Page<SysYfyd> page,Map<String, Object> map) {
        page.setRecords(sysYfydDao.queryPagecolList(page, map));
        return page;
    }


    @Override
    public Page<SysYfyd> queryPagednfsdxfjsList(Page<SysYfyd> page, Map<String, Object> map) {
        return page.setRecords(sysYfydDao.queryPagednfsdxfjsList(page, map));
    }

    @Override
    public Page<SysYfyd> queryPagednwcylnwczhList(Page<SysYfyd> page, Map<String, Object> map) {
        return page.setRecords(sysYfydDao.queryPagednwcylnwczhList(page, map));
    }

    @Override
    public void deleteBatch(String[] yfydIds) {
        sysYfydDao.deleteBatch(yfydIds);
    }
    ...
    public List<Map<String, Object>> getList(String groupId) {
        ...
        List<SysYfyd> yfydList = sysYfydDao.selectList(new EntityWrapper<SysYfyd>().orderBy(true,"updatetime"));
        int i =0;
        for(SysYfyd sysYfyd : yfydList) {
            Map<String, Object> GroupXnMap = new HashMap<String, Object>();
//            GroupXnMap.put("xu", ++i);
            GroupXnMap.put("xh", sysYfyd.getXh());
            GroupXnMap.put("sf", sysYfyd.getSf());
            GroupXnMap.put("nf", sysYfyd.getNf());
            GroupXnMap.put("kl", sysYfyd.getKl());
            GroupXnMap.put("fs", sysYfyd.getFs());
            GroupXnMap.put("bdrs", sysYfyd.getBdrs());
            GroupXnMap.put("ljrs", sysYfyd.getLjrs());
