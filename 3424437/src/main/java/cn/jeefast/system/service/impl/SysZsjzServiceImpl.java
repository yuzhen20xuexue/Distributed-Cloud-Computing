...
import cn.jeefast.system.dao.SysZsjzDao;
 ...
 */
@Service
public class SysZsjzServiceImpl extends ServiceImpl<SysZsjzDao, SysZsjz> implements SysZsjzService {
    @Autowired
    private SysZsjzDao sysZsjzDao;

    @Override
    public Page<SysZsjz> queryPageList(Page<SysZsjz> page, Map<String, Object> map) {
        page.setRecords(sysZsjzDao.queryPageList(page, map));
        return page;
    }

    @Override
    public void deleteBatch(String[] zsjzIds) {
        sysZsjzDao.deleteBatch(zsjzIds);
    }
    ...
    public List<Map<String, Object>> getList(String groupId) {
        ...
        List<SysZsjz> sysZsjzs = sysZsjzDao.selectList(new EntityWrapper<SysZsjz>().orderBy(true,"updatetime"));
        int i =0;
        for(SysZsjz sysZsjz : sysZsjzs) {
            Map<String, Object> GroupXnMap = new HashMap<String, Object>();
//            GroupXnMap.put("xu", ++i);
            GroupXnMap.put("xuexiao", sysZsjz.getXuexiao());
            GroupXnMap.put("address", sysZsjz.getAddress());
