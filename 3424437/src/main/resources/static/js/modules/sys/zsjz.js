...
var vm = new Vue({
    ...
    methods: {
        otherWorkToExcel: function() {
            window.top.location.href = baseURL + "sysZsjz/otherWorkToExcel?name=dc"+"&token="+token;
        },
        readExcel: function(){
        ...
        add: function () {
            ...
            vm.equipmentList = {};
            vm.title = "新增";
            vm.zsjz = {deptName: null, deptId: null};
            vm.getDept();
        },
        update: function () {
        ...
        getZsjz: function (zsjzId) {
            $.get(baseURL + "sysZsjz/info/" + zsjzId, function (r) {
                vm.zsjz = r.zsjz;
                vm.allFiles = r.zsjz.files;
                vm.getDept();
            });
        },
        saveOrUpdate: function () {
            var url = vm.zsjz.id == null ? "sysZsjz/save" : "sysZsjz/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.zsjz),
                success: function (r) {
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.reload();
        ...
        getDept: function () {
            ...
            $.get(baseURL + "sys/dept/list", function (r) {
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.zsjz.deptid);
                if (node != null) {
                    dept_ztree.selectNode(node);

                    vm.zsjz.deptName = node.name;
                }
            })
        },
        deptTree: function () {
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.zsjz.deptid = node[0].deptId;
                    vm.zsjz.deptName = node[0].name;
