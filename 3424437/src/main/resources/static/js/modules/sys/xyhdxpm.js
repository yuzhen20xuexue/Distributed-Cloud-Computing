$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sysXyhdxpm/list',
        datatype: "json",
        colModel: [
            {label: 'ID', name: 'id', index: "id", width: 45, key: true},
            {label: '序号', name: 'xh', index: "xh", width: 100},
            {label: '学校名次', name: 'xxmc', index: "xxmc", width: 100},
            {label: '学校名称', name: 'xuexiao', index: "xuexiao", width: 100},
            {label: '总分', name: 'zf', index: "zf", width: 100},
            {label: '星级', name: 'xj', index: "xj", width: 100},
            {label: '办学层次', name: 'bxcc', index: "bxcc", width: 100},
        ...
        onSortCol: function (index, columnIndex, sortOrder) {
            ...
            $("#jqGrid").jqGrid("setGridParam",{
                url: baseURL + 'sysXyhdxpm/collist',
                datatype: "json",
...
var vm = new Vue({
    ...
    methods: {
        otherWorkToExcel: function() {
            window.top.location.href = baseURL + "sysXyhdxpm/otherWorkToExcel?name=dc"+"&token="+token;
        },
        readExcel: function(){
            ...
            $.ajax({
                ...
                success: function (r) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: baseURL + "sysXyhdxpm/readExcel",
                        contentType: "application/json",
        ...
        add: function () {
            ...
            vm.equipmentList = {};
            vm.title = "新增";
            vm.xyhdxpm = {deptName: null, deptId: null};
            vm.getDept();
        },
        update: function () {
        ...
        getXyhdxpm: function (xyhdxpmId) {
            $.get(baseURL + "sysXyhdxpm/info/" + xyhdxpmId, function (r) {
                vm.xyhdxpm = r.xyhdxpm;
                vm.allFiles = r.xyhdxpm.files;
                vm.getDept();
            });
        },
        saveOrUpdate: function () {
            var url = vm.xyhdxpm.id == null ? "sysXyhdxpm/save" : "sysXyhdxpm/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify(vm.xyhdxpm),
                success: function (r) {
                    if (r.code === 0) {
                        alert('操作成功', function () {
                            vm.reload();
        ...
        getDept: function () {
            ...
            $.get(baseURL + "sys/dept/list", function (r) {
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.xyhdxpm.deptid);
                if (node != null) {
                    dept_ztree.selectNode(node);

                    vm.xyhdxpm.deptName = node.name;
                }
            })
        },
        deptTree: function () {
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.xyhdxpm.deptid = node[0].deptId;
                    vm.xyhdxpm.deptName = node[0].name;
        ...
        reload: function () {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {'name': vm.q.xyhdxpmName},
                page: page
            }).trigger("reloadGrid");
        },
